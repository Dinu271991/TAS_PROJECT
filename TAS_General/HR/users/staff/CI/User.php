...
class Cl_User
{
		...
		if( !empty( $data ) ){
			...
			if(mysqli_query($this->_con, $query)){
				...
				$headers = "From: admin@vetbossel.in" . "\r\n" .
						"CC: dineshr@suntechnologies.com";
					//admin@vetbossel.in
	...
	{
		$query = "SELECT * FROM `tbl_mcq_category`";
	...
	public function getQuestions(array $data)
	{
		if( !empty( $data ) ){
			...
			$row = mysqli_query( $this->_con, "select * from mcq where category_id=$category_id ORDER BY RAND()");
	...
	public function getPrgmQuestions(array $data)
	{
		if( !empty( $data ) ){
			...
			$results1 = array();
			$number_question = $_POST['num_questions'];
			$row = mysqli_query( $this->_con, "select * from tbl_programs where category_id=$category_id ORDER BY RAND()");
			$rowcount = mysqli_num_rows( $row );
			$remainder = $rowcount/$number_question;
			$results1['number_question'] = $number_question;
			$results1['remainder'] = $remainder;
			$results1['rowcount'] = $rowcount;
			while ( $result1 = mysqli_fetch_assoc($row) ) {
				
				$results1['program_id'][] = $result1;
