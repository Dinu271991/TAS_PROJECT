...
require_once 'dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_mobile_computing");
 $stmt->execute();
 $count = $stmt->rowCount();
	  ...
	  while($row=$stmt->fetch(PDO::FETCH_ASSOC))
	  {
		 // echo "Entered into while...";
		  //$row= mysql_fetch_row($rs);
			...
			$pid=$row['mobile_computing_id'];
			//echo $pid;
			$query="Select * from tbl_user_mobile_computing where uid=$id and mc_id=$pid";
			...
			$count2 = $stmt2->rowCount();
				...
				{
					//echo "Updating...";
					$query3="UPDATE tbl_user_mobile_computing SET uid='$id',mc_id=$pid,mc_desc='$selected'  WHERE mc_id=$pid and uid='$id'";
				...
				{
					//echo "Inserting...";
					$query3="insert into tbl_user_mobile_computing(uid,mc_id,mc_desc) values ('$id',$pid,'$selected')";
...
<div class="container">
  <span style='font-family:arial;color:green;font-size:25px;'>Your answers for Mobile Computing Section has been submitted Successfully.</span>
</div>
