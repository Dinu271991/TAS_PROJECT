...
require_once 'dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_datastructure");
 $stmt->execute();
 $count = $stmt->rowCount();
	  ...
	  while($row=$stmt->fetch(PDO::FETCH_ASSOC))
	  {
		 // echo "Entered into while...";
		  //$row= mysql_fetch_row($rs);
			...
			$pid=$row["ds_id"];
			//echo $pid;
			$query="Select * from tbl_user_ds where uid=$id and ds_id=$pid";
			...
			$count2 = $stmt2->rowCount();
				...
				{
					//echo "Updating...";
					$query3="UPDATE tbl_user_ds SET uid='$id', ds_id=$pid, ds_desc='$selected'  WHERE ds_id=$pid and uid='$id'";
				...
				{
					//echo "Inserting...";
					$query3="insert into tbl_user_ds(uid,ds_id,ds_desc) values ('$id',$pid,'$selected')";
...
<div class="container">
  <span style='font-family:arial;color:green;font-size:25px;'>Your answers for Data Structure Section has been submitted Successfully.</span>
</div>
