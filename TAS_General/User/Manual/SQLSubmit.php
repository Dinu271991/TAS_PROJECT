...
require_once 'dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_manual_sql");
 $stmt->execute();
 $count = $stmt->rowCount();
	...
	//echo "program session set!!";
	  while($row=$stmt->fetch(PDO::FETCH_ASSOC))
	  {
		 // echo "Entered into while...";
		  //$row= mysql_fetch_row($rs);
			$selected=$_POST['sql'.$i];
			//echo($i.". Selected value is: ".$selected."<br/>");
			echo "Text is: ".$selected."<br/>";
		
			$pid=$row['sql_id'];
			echo $pid;
			$query="Select * from tbl_user_sql where uid=$id and sql_id=$pid";
			...
			$count2 = $stmt2->rowCount();
				...
				{
					//echo "Updating...";
					$query3="UPDATE tbl_user_sql SET uid='$id',sql_id=$pid,sql_desc='$selected'  WHERE sql_id=$pid and uid='$id'";
				...
				{
					//echo "Inserting...";
					$query3="insert into tbl_user_sql(uid,sql_id,sql_desc) values ('$id',$pid,'$selected')";
...
<div class="container">
  <span style='font-family:arial;color:green;font-size:25px;'>Your answers for SQL Section has been submitted Successfully.</span>
</div>
