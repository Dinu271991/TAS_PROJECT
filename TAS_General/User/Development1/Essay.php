...
require_once 'dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_development_essay ORDER BY rand()");
 $stmt->execute();
 $count = $stmt->rowCount();
		...
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {		
							...
							<?php  
								...
								if($row['essay_id']==1)
								{
								echo $k." ".$row['essay_desc']."<br/>"."<br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='15' cols='100' name='browsint<?php echo $row['essay_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php }
								else
								{
