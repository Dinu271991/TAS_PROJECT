<h3>You are under Technical Section!!</h3>

<HTML>

<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>Development Aptitiude</title>
...
$(document).ready(function () {
	...
	$("#Save").click(function(){
$.post("tempTech.php",$("#quiz_form").serialize(),function(response,status){ // Required Callback Function
...
<body>
 <button id="Save" class="save" type="button" style="position:fixed;top:0%;left:80%;">Save</button>
...
<?php
	...
	$stmt = $db_con->prepare("SELECT * FROM tbl_development_technical ORDER BY rand()");
    $stmt->execute();							//$number_question =  2;
	$number_question =  3;
	$rowcount = $stmt->rowCount();
	$remainder = ($rowcount)/($number_question);
	
	echo "<span id='maxQuest'><b>No of Quest:: ".$rowcount."</b></span><br/>";
	...
	<form class="form-horizontal" role="form" id='quiz_form' method="post" action="TechnicalSubmit.php">
						<?php
						...
						while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {
							...
							<p class='questions' id="qname<?php echo $j;?>"> <?php echo "<b><pre style='font-size:17px;'>".$k?>.<?php echo $row['qdesc']."</pre></b>";?></p>
