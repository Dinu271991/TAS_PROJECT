<h3>You are under Java Program/Algorithm Section!!</h3>
...
$(document).ready(function () {
	...
	$("#Save").click(function(){
$.post("tempJavaAlgo.php",$("#myFrm").serialize(),function(response,status){ // Required Callback Function
...
<form name="home" action="JavaProgSubmit.php" id="myFrm" method="POST">
...
require_once 'dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_javaprog_algo ORDER BY rand()");
 $stmt->execute();
 $count = $stmt->rowCount();
 ...
 echo "<br/>";
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {		
							...
							<div class="columns" id='JavaAlgo<?php echo $k;?>' >
									
							<?php  
							
								if($row['java_or_algo_id']==1)
								{
								echo $k." ".$row['java_or_algo_desc']."<br/>"."<br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='10' cols='100' name='javaAlgo<?php echo $row['java_or_algo_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php }
								else
								{
