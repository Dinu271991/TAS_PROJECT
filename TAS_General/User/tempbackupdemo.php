...
<button id="Done" name="Done" class='btn btn-danger' style="position:fixed;top:17%;right:7%;" onClick="myFunction();">Abort Test</button>
...
<img src="images1/descriptive.png" class="options" id="descriptive"  alt="Descriptive" border="0" height="220px" width="240px" style="position:fixed;left:1%;"/>	
	  ...
	  <!-- <div id="iframeSections" style="position:absolute;left:90%">
        ...
        //$automation=array("ViewChoice.php","Program.php","Essay.php","HTML.php");
			$stmt1 = $db_con->prepare("SELECT * FROM tbl_mcq_category where DeptId=2");
			$stmt1->execute();
			$j=0;	
			$n=5;
			$top=0;
			...
			while($row1=$stmt1->fetch(PDO::FETCH_ASSOC))
			{
				...
				$imgCateg=$categName1[$j].".png";
				$modalTarget="#".$modalName;
				echo "<div class=gallery>";
				
				if($row1['category_name']!="Aptitude")
				{
				$n+=18;
				$top=32;
				echo "<img src=$imgCateg class='descr' data-toggle=modal data-target=$modalTarget id=$strReplace alt='Descriptive' border='0' height='300px' width='300px' style='position:fixed;top:$top%;left:$n%;' hspace='80px'/></img>";
				...
				$n+=18;
				$top=32;
				echo "<img src=$imgCateg class='aptitude' data-toggle=modal data-target=$modalTarget id=$strReplace alt='Descriptive' border='0' height='150px' width='200px' style='position:fixed;top:$top%;left:41%;' hspace='80px'/></img>";
			   	//echo "<div class=desc style='position:absolute;top:$textTop%;left:$n%;font-size:18px;'><b>$strReplace</b></div>";
				$n=5;
				}
				echo "</div>";
	 ...
	 <div class="modal-dialog" role="document" style="position:absolute;top:15%;left:18%;width:80%;">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" id="modal_close" class="close" aria-label="Close"><span aria-hidden="true">&times;</span></button>
          <h4 class="modal-title" id="myModalLabel"><?php echo $row1['category_type']; ?>  Section - <?php echo "<span style='color:blue;'>".$strReplace."</span>"; ?></h4>
        </div>
...
$(document).ready(function()
{
	 ...
	 $(".close").click(function(){
		...
		//subSectionTest();
         ...
         success:function(response){
			 if(response==1)
			 {
				 ...
				 $(".close").attr('data-dismiss','modal');
				  $(".close").click();
				alert("Programming section has been already submitted!!");				
		...
		//alert("Your data is temporarily available.. until you click Save or Final Submit");
