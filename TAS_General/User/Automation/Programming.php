    ...
    //echo "<br/>";
		...
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {		
							...
							<?php  
								echo "<br/><span class='questions'>".$k." ".$row['prog_desc']."</span><br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='25' cols='100' id='prog<?php echo $row['program_id'];?>' name='prog<?php echo $row['program_id'];?>' class="programs" style='resize: none;'></textarea>
							...
							<?php 
								 ...
								 if ( ( $remainder < 1 ) || ( $i == $number_question && $remainder == 1 ) ) {
									...
									echo "<button id='Save' class='save' type='button' style=position:fixed;top:0%;left:93%;>Save <i class='fa fa-floppy-o' aria-hidden='true'></i></button>";
