...
<form name="home" id="myFrm" action="ProgSubmit.php" method="POST">
...
require_once '../dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_manual_programming ORDER BY rand()");
 $stmt->execute();
 $count = $stmt->rowCount();
 ...
 echo "<br/>";
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {		
							...
							<div class="columns" id='prog<?php echo $k;?>' >
									
							<?php  
							
								if($row['prog_id']==1)
								{
								echo $k." ".$row['prog_desc']."<br/>"."<br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='20' cols='105' name='prog<?php echo $row['prog_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php }
								else
								{
								 echo $k." ".$row['prog_desc']."<br/>"."<br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='5' cols='95' name='prog<?php echo $row['prog_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php } ?>								
								</div> 
						<?php 
