...
require_once 'dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_manual_notes");
 $stmt->execute();
 $count = $stmt->rowCount();
	...
	//echo "program session set!!";
	  while($row=$stmt->fetch(PDO::FETCH_ASSOC))
	  {
		 // echo "Entered into while...";
		  //$row= mysql_fetch_row($rs);
			$selected=$_POST['notes'.$i];
			//echo($i.". Selected value is: ".$selected."<br/>");
			//echo "Text is: ".$selected."<br/>";
		
			$pid=$row['notes_id'];
			//echo $pid;
			$query="Select * from tbl_user_notes where uid=$id and notes_id=$pid";
			...
			$count2 = $stmt2->rowCount();
				...
				{
					//echo "Updating...";
					$query3="UPDATE tbl_user_notes SET uid='$id',notes_id=$pid,notes_desc='$selected'  WHERE notes_id=$pid and uid='$id'";
				...
				{
					//echo "Inserting...";
					$query3="insert into tbl_user_notes(uid,notes_id,notes_desc) values ('$id',$pid,'$selected')";
