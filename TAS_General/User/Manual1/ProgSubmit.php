...
require_once 'dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_manual_programming");
 $stmt->execute();
 $count = $stmt->rowCount();
	...
	//echo "program session set!!";
	  while($row=$stmt->fetch(PDO::FETCH_ASSOC))
	  {
		...
		//	echo "Text is: ".$selected."<br/>";
			...
			$pid=$row["prog_id"];
			//echo $pid;
			$query="Select * from tbl_user_manual_program where uid=$id and prog_id=$pid";
			...
			$count2 = $stmt2->rowCount();
				...
				{
					//echo "Updating...";
					$query3="UPDATE tbl_user_manual_program SET uid='$id', prog_id=$pid, prog_desc='$selected'  WHERE prog_id=$pid and uid='$id'";
				...
				{
					//echo "Inserting...";
					$query3="insert into tbl_user_manual_program(uid,prog_id,prog_desc) values ('$id',$pid,'$selected')";
...
<div class="container">
  <span style='font-family:arial;color:green;font-size:25px;'>Your answers for Programming Section has been submitted Successfully.</span>
</div>
