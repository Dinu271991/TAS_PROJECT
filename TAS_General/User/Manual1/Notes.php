<h3>You are under Notes Section!!</h3>
...
$(document).ready(function () {
	...
	$("#Save").click(function(){
$.post("tempEssay.php",$("#myFrm").serialize(),function(response,status){ // Required Callback Function
...
<form name="home" id="myFrm" action="NotesSubmit.php" method="POST">
...
require_once '../dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_manual_notes ORDER BY rand()");
 $stmt->execute();
 $count = $stmt->rowCount();
 ...
 echo "<br/>";
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {		
							...
							<div class="columns" id='Essay<?php echo $k;?>' >
									
							<?php  
							
								if($row['notes_id']==1)
								{
								echo $k." ".$row['notes_desc']."<br/>"."<br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='15' cols='115' name='essay<?php echo $row['notes_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php }
								else
								{
								 echo $k." ".$row['notes_desc']."<br/>"."<br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='5' cols='100' name='essay<?php echo $row['notes_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php } ?>								
								</div> 
						<?php 
								 ...
								 if ( ( $remainder < 1 ) || ( $i == $number_question && $remainder == 1 ) ) {
								 	...
								 	echo "<button id='Save' class='save' type='button' style=position:fixed;left:80%;>Save</button>";
