<h3>You are under Operating System Section!!</h3>
<?php
/*echo '<html>
<head>
<style type="text/css">
...
$(document).ready(function () {
	...
	$("#Save").click(function(){
$.post("tempOS.php",$("#myFrm").serialize(),function(response,status){ // Required Callback Function
...
<form name="home" id="myFrm" action="OSSubmit.php" method="POST">
...
require_once '../dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_manual_os ORDER BY rand()");
 $stmt->execute();
 $count = $stmt->rowCount();
 ...
 echo "<br/>";
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {		
							...
							<div class="columns" id='osManual<?php echo $k;?>' >
									
							<?php  
							
								if($row['os_id']==1)
								{
								echo $k." ".$row['os_desc']."<br/>"."<br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='5' cols='100' name='os<?php echo $row['os_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php }
								else
								{
