<h3>You are under General Computing Section!!</h3>
<?php
/*echo '<html>
<head>
<style type="text/css">
...
$(document).ready(function () {
	...
	$("#Save").click(function(){
$.post("tempGC.php",$("#myFrm").serialize(),function(response,status){ // Required Callback Function
...
<form name="home" id="myFrm" action="GCSubmit.php" method="POST">
...
require_once '../dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_general_computing ORDER BY rand()");
 $stmt->execute();
 $count = $stmt->rowCount();
 ...
 echo "<br/>";
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {		
							...
							<div class="columns" id='GenComp<?php echo $k;?>' >
									
							<?php  
							
								if($row['gc_id']==1)
								{
								echo "<pre style='font-size:17px;'>".$k." ".$row['gc_desc']."</pre>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='5' cols='100' name='gc<?php echo $row['gc_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php }
								else
								{
