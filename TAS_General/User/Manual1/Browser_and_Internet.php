<h3>You are under Browser and Internet Section!!</h3>
...
$(document).ready(function () {
	...
	$("#Save").click(function(){
$.post("tempB&I.php",$("#myFrm").serialize(),function(response,status){ // Required Callback Function
...
<form name="home" id="myFrm" action="BrowserInternetSubmit.php" method="POST">
...
require_once '../dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_browser_and_internet ORDER BY rand()");
 $stmt->execute();
 $count = $stmt->rowCount();
 ...
 echo "<br/>";
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {		
							...
							<div class="columns" id='BrowserInternet<?php echo $k;?>' >
									
							<?php  
							
								if($row['browser_internet_id']==1)
								{
								echo $k." ".$row['browser_internet_desc']."<br/>"."<br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='5' cols='100' name='browsint<?php echo $row['browser_internet_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php }
								else
								{
