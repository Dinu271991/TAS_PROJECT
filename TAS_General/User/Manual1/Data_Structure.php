<h3>You are under Data Structure Section!!</h3>
...
$(document).ready(function () {
	...
	$("#Save").click(function(){
$.post("tempDS.php",$("#myFrm").serialize(),function(response,status){ // Required Callback Function
...
<form name="home" id="myFrm" action="DSSubmit.php" method="POST">
...
require_once '../dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_datastructure ORDER BY rand()");
 $stmt->execute();
 $count = $stmt->rowCount();
 ...
 echo "<br/>";
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {		
							...
							<div class="columns" id='DS<?php echo $k;?>' >
									
							<?php  
							
								if($row['ds_id']==1)
								{
								echo $k." ".$row['ds_desc']."<br/>"."<br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='10' cols='95' name='ds<?php echo $row['ds_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php }
								else
								{
								 echo $k." ".$row['ds_desc']."<br/>"."<br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='5' cols='95' name='ds<?php echo $row['ds_id'];?>' style='resize: none;'></textarea>
								<br/><br/><br/>
								<?php } ?>								
								</div> 
						<?php 
