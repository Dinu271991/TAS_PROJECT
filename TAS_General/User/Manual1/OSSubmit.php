...
require_once 'dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_manual_os");
 $stmt->execute();
 $count = $stmt->rowCount();
	  ...
	  while($row=$stmt->fetch(PDO::FETCH_ASSOC))
	  {
		 // echo "Entered into while...";
		  //$row= mysql_fetch_row($rs);
			...
			$pid=$row['os_id'];
			//echo $pid;
			$query="Select * from tbl_user_os where uid=$id and os_id=$pid";
			...
			$count2 = $stmt2->rowCount();
				...
				{
					//echo "Updating...";
					$query3="UPDATE tbl_user_os SET uid='$id',os_id=$pid,os_desc='$selected'  WHERE os_id=$pid and uid='$id'";
				...
				{
					//echo "Inserting...";
					$query3="insert into tbl_user_os(uid,os_id,os_desc) values ('$id',$pid,'$selected')";
...
<div class="container">
  <span style='font-family:arial;color:green;font-size:25px;'>Your answers for Operating System Section has been submitted Successfully.</span>
</div>
