...
<body style="font-family:none;">
 ...
 <button id="Save" class="save" type="button" style="position:fixed;top:0%;left:93%;">Save <i class='fa fa-floppy-o' aria-hidden='true'></i></button>
<?php
require_once 'dbconfig.php';
	...
	<form class="form-horizontal" role="form" id='quiz_form' name="home" method="post" action="AptiSubmit.php">
						...
						while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {
							...
							<input type="radio" value="1" id='radio1_<?php echo $row['qid'];?>' name="ans<?php echo $row['qid'];?>" class="option" /><?php echo $row['option1'];?>
							<br/>
							<input type="radio" value="2" id='radio1_<?php echo $row['qid'];?>' name="ans<?php echo $row['qid'];?>" class="option" /><?php echo $row['option2'];?>
							<br/>
							
							<?php if(isset( $row['option3'] ) && !empty( $row['option3'] )){ ?>
							<input type="radio" value="3" id='radio1_<?php echo $row['qid'];?>' name="ans<?php echo $row['qid'];?>" class="option" /><?php echo $row['option3'];?>
							<br/>
							<?php } ?>
							
							<?php if(isset( $row['option4'] ) && !empty( $row['option4'] )){ ?>
							<input type="radio" value="4" id='radio1_<?php echo $row['qid'];?>' name="ans<?php echo $row['qid'];?>" class="option" /><?php echo $row['option4'];?>
							<br/>
							<?php } ?>
							
							<?php if(isset( $row['option5'] ) && !empty( $row['option5'] )){ ?>
							<input type="radio" value="5" id='radio1_<?php echo $row['qid'];?>' name="ans<?php echo $row['qid'];?>" class="option" /><?php echo $row['option5'];?>
							<br/>
							<?php } ?>
							
							<?php if(isset( $row['option6'] ) && !empty( $row['option6'] )){ ?>
							<input type="radio" value="6" id='radio1_<?php echo $row['qid'];?>' name="ans<?php echo $row['qid'];?>" class="option" /><?php echo $row['option6'];?>
							<br/>
							<?php } ?>
