...
var essay3 = $("#essay3").val();
var essay4 = $("#essay4").val();
var essay5 = $("#essay5").val();
...
<form name="home" action="EssaySubmit.php" id="myFrm" method="POST">
...
require_once 'dbconfig.php';
 ...
 $stmt = $db_con->prepare("SELECT * FROM tbl_essays ORDER BY RAND() ");
 $stmt->execute();
 $count = $stmt->rowCount();
...
//echo "You are in Essay Section<br/><br/>";	
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					    {		
							...
							<?php  
								echo "<br/><span class='questions'>".$k." ".$row['essay_desc']."</span><br/>"; ?>
								<textarea autocomplete='off' autocorrect='off' autocapitalize='off' spellcheck='false' rows='15' cols='100' id='essay<?php echo $row['essay_id'];?>' name='essay<?php echo $row['essay_id'];?>' class="essays" style='resize: none;'></textarea>
								<br/><br/>							
								</div> 
						<?php 
								  $i++; 
