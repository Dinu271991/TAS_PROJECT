...
<script>
  function handleChange(input,customValue) {
    if (input.value < 0) input.value = 0;
    if (input.value > customValue) input.value = customValue;
  }
</script>
<style>
...
#dis{
	...
	<tr style="height:20px;"></tr>
		<?php
			...
			$stmt = $db_con->prepare("SELECT prog_score from tbl_programs");
			$stmt->execute();
			...
			while($row=$stmt->fetch(PDO::FETCH_ASSOC))
			{
				...
				echo "<tr><td>Programming Score:</td><td><input type=number name='Program$row[program_id]' id='Program$row[program_id]' class='form-control' value='$row[prgm_score]' onchange='handleChange(this,$progScore);'/></td></tr>";
				echo "</table>";
				echo "<br/>";
				$y++;
